...
func (s *AppService) SendMessageStream(sessionID, userText string) map[string]any {
	...
	go s.runStream(sessionID, userText)
	return map[string]any{"success": true, "streaming": true, "session_id": sessionID}
}

func (s *AppService) runStream(sessionID, userText string) {
	if sessionID != "" {
		pctx := context.Background()
		_ = s.rt.Sessions().AppendMessage(pctx, sessionID, "user", userText, "text", nil)
	...
	res = s.sendMessageCore(ctx, sessionID, userText, emit)
