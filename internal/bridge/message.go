...
func (s *AppService) sendMessageCore(ctx context.Context, sessionID, userText string, streamEmit func(string)) SendMessageResult {
	...
	var err error
	if streamEmit != nil {
		runRes, err = runner.GenerateStream(ctx, llm, sessionID, userText, streamEmit)
	} else {
		runRes, err = runner.Generate(ctx, llm, sessionID, userText)
	}
	if err != nil {
